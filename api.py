"""
api.py — Production version (uses ONNX Runtime, not full PyTorch)
Much lighter: ~50MB RAM vs ~700MB for PyTorch.
Falls back to PyTorch if ONNX not found.
"""
import sys
...
# ── Model globals ──
SESSION       = None   # ONNX session (production)
TORCH_MODEL   = None   # PyTorch fallback
USE_ONNX      = False
MODEL_ACCURACY = 0.0
...
def load_onnx():
    ...
    try:
        ...
        if Path("digit_model.onnx").exists():
            ...
            USE_ONNX = True
            logger.info("[OK] ONNX model loaded (lightweight mode)")
            return True
    except ImportError:
        pass
    return False
    ...
    try:
        ...
        class DigitCNN(nn.Module):
            def __init__(self):
                ...
                self.classifier = nn.Sequential(nn.Linear(64*7*7,128), nn.ReLU(inplace=True), nn.Dropout(0.3), nn.Linear(128,10))
        ...
        if Path("digit_model.pth").exists():
            ...
            m = DigitCNN(); m.load_state_dict(ck["model_state_dict"]); m.eval()
            TORCH_MODEL = m
            MODEL_ACCURACY = ck.get("accuracy", 0.0)
            logger.info("[OK] PyTorch model loaded (fallback mode)")
            return True
...
def preprocess(image_bytes: bytes) -> np.ndarray:
    ...
    # User draws dark strokes on WHITE canvas → mean > 128 → invert to match MNIST
...
def run_inference(tensor: np.ndarray) -> np.ndarray:
    if USE_ONNX:
        return SESSION.run(["output"], {"input": tensor})[0][0]
    import torch
    with torch.no_grad():
...
@app.get("/health")
async def health():
    return {"status": "ok", "model_loaded": USE_ONNX or TORCH_MODEL is not None,
            "mode": "onnx" if USE_ONNX else "pytorch",
            "model_accuracy": round(MODEL_ACCURACY * 100, 2), "device": "cpu"}

@app.post("/predict")
async def predict(file: UploadFile = File(...)):
    if not (USE_ONNX or TORCH_MODEL):
        raise HTTPException(503, "No model loaded. Run python train_model.py && python export_onnx.py")
    try:
        ...
        return {"digit": digit, "confidence": round(float(probs[digit])*100, 2),
                "probabilities": [round(float(p)*100, 2) for p in probs]}
    except ValueError as e: raise HTTPException(400, str(e))
    except Exception as e: logger.error(e); raise HTTPException(500, "Prediction error")

@app.get("/metrics")
async def metrics():
    p = Path("static/history.json")
    return json.loads(p.read_text()) if p.exists() else {"accuracy": [], "val_accuracy": [], "loss": [], "val_loss": [], "final_accuracy": 0, "epochs": 0}
